...
function createAdmin($request_values){
    ...
    if (empty($first_name)) { array_push($errors, "Uhmm...We gonna need the firstname"); }
    if (empty($last_name)) { array_push($errors, "Uhmm...We gonna need the lastname"); }
    if (empty($gender)) { array_push($errors, "Uhmm...We gonna need the gender"); }
...
//		$password = md5($password);//encrypt the password before saving in the database
        $query = "INSERT INTO users (username,first_name,last_name,gender, email, role, password, created_at, updated_at) 
				  VALUES('$username','$first_name' ,'$last_name','$gender','$email', '$role', '$password', now(), now())";
...
{
    global $conn, $username,$first_name ,$last_name,$gender,$role, $isEditingUser, $admin_id, $email;
